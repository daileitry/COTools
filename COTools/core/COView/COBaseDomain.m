...
- (void)parseWithClass:(Class )mClass1 withJsonContent:(NSDictionary *)json{ // null
...
//        NSLog(@"property_getName Key : ============ > %@  %@", key, key.class);
        if ([key isEqualToString:@"media"] || [key isEqualToString:@"offer"] || [key isEqualToString:@"cancel"] ||  [key isEqualToString:@"targetDispute"]) {
        }
...
//        } || [value isEqualToString:@"(null)"]
        //如果不存在,则查看是否有新的key
        NSString *nullValue = [NSString stringWithFormat:@"%@", value];
        if ([nullValue isEqualToString:@"transportation"] || [nullValue isEqualToString:@"profile"]) {
            
        }
        if (value == [NSNull null] || [nullValue isEqualToString:@"(null)"]) {
            value = nil;
        }
        if (!value) {
