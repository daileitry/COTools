...
+ (float )widthWithContent:(NSString *)content withFont:(UIFont *)font withWidth:(float )width{
    if([content respondsToSelector:@selector(boundingRectWithSize:options:attributes:context:)]){
        NSMutableDictionary *atts = [[NSMutableDictionary alloc] init];
        [atts setObject:font forKey:NSFontAttributeName];
        ...
        return [content sizeWithFont:font  constrainedToSize:CGSizeMake(width,100000)  lineBreakMode:NSLineBreakByWordWrapping].width;
    }
}

- (void)setUnderLineAttr{
    NSMutableAttributedString *attributionStr = [[NSMutableAttributedString alloc] initWithString:self.text];
    NSRange range = [self.text rangeOfString:self.text];
    [attributionStr addAttribute:NSUnderlineStyleAttributeName value:[NSNumber numberWithInteger:NSUnderlineStyleSingle] range:range];
    self.attributedText = attributionStr;
}
