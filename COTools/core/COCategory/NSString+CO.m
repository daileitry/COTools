...
- (NSString *)verifyStr:(NSString *)regexStr{
    if (!regexStr) {
        return nil;
    }
    NSError *error = nil;
    ...
    if (matches != nil && matches.count>0) {
        ...
        NSRange resultRange = [checkingResult rangeAtIndex:0];
...
- (NSArray *)verifyArrayStr:(NSString *)regexStr{
    if (!regexStr) {
        return nil;
    }
    NSError *error = nil;
    ...
    if (matches != nil && matches.count>0) {
        for (int index = 0; index<matches.count; index++) {
            NSTextCheckingResult *checkingResult = matches[index];
            NSRange resultRange = [checkingResult rangeAtIndex:0];
            NSString *resultStr=[self substringWithRange:resultRange];
